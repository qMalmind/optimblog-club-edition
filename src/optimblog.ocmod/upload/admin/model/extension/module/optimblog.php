...
class ModelExtensionModuleOptimBlog extends Model {
	public function createTables() {
		...
		foreach ($field_query->rows as $field) {
			if ($field['Field'] == 'header' && $field_name != 'name') {
				$this->db->query("ALTER TABLE `" . DB_PREFIX . "product_description` CHANGE `short_description` `short_description` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `meta_h1`");
			}
